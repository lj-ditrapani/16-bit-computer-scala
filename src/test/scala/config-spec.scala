    ...
    describe("load") {
      ...
      describe("--f") {
        it("returns Left if --f not set") {
          Config.load(List(), Map()) shouldBe Left(
        ...
        it("returns a Left if --f is not a file") {
          ...
          Config.load(List(), Map("f" -> prefix)) shouldBe Left(msg)
        }
        ...
        it("returns a Left if file is not made of only 16-bit values") {
          ...
          Config.load(List(), Map("f" -> bin_path)) shouldBe Left(msg)
        }
      ...
      describe("--m") {
        it("returns a Left if --m not a number") {
          Config.load(List(), Map("f" -> bin_path, "m" -> "foo")) shouldBe Left(
            "--m must be an integer between 1 and 64"
          )
        }

        it("returns a Left if --m is 0") {
          Config.load(List(), Map("f" -> bin_path, "m" -> "0")) shouldBe Left(
            "--m must be an integer between 1 and 64"
          )
        }

        it("returns a Left if --m is negative") {
          Config.load(List(), Map("f" -> bin_path, "m" -> "-1")) shouldBe Left(
            "--m must be an integer between 1 and 64"
          )
        }
      }
